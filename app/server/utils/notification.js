...
var GOOGLE_UTIL = require('./google');
...
module.exports = {
    sms: {
        ...
        sendShortUrl: function (phoneNum, url, message, callback) {
            if (sendNoti.sms) {
                GOOGLE_UTIL.shorten(url, function (status, shortUrl) {
                    if (status == 200) {
                        message = message + '\n' +  shortUrl;
                        sendNoti.sms(null, phoneNum, '', message, function (err) {
                            if (err) {
                                console.error(err);
                            }
                            callback();
